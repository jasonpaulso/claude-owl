...
export function registerCCUsageHandlers() {
  ...
  process.stdout.write(`[CCUsage] CHECK_CCUSAGE_INSTALLED channel: ${CHANNELS.CHECK_CCUSAGE_INSTALLED}\n`);

  // Check if ccusage is installed
  ipcMain.handle(CHANNELS.CHECK_CCUSAGE_INSTALLED, async (): Promise<CheckCCUsageInstalledResponse> => {
    try {
      ...
      process.stderr.write(`[CCUsage IPC] Error in check handler: ${error instanceof Error ? error.message : error}\n`);
      return {
        success: false,
        installed: false,
  ...
  ipcMain.handle(CHANNELS.GET_USAGE_REPORT, async (): Promise<GetUsageReportResponse> => {
    try {
      ...
      process.stderr.write(`[CCUsage IPC] Error getting usage report: ${error instanceof Error ? error.message : error}\n`);
      return {
        success: false,
  ...
  ipcMain.handle('ccusage:get-raw-output', async (): Promise<{ success: boolean; data?: string; error?: string }> => {
    try {
      ...
      process.stderr.write(`[CCUsage IPC] Error getting raw output: ${error instanceof Error ? error.message : error}\n`);
      return {
        success: false,
