...
export function usePlugins() {
  ...
  const loadPluginData = useCallback(async () => {
    ...
    try {
      ...
      const marketplaces = marketplacesRes.success && marketplacesRes.data ? marketplacesRes.data : [];
      ...
      const enrichedAvailablePlugins = availablePlugins.map(plugin => {
        const installedPlugin = installedPlugins.find(p => p.id === `${plugin.name}@${plugin.marketplace}`);
        return {
          ...plugin,
   ...
   */
  const addMarketplace = useCallback(async (name: string, source: string): Promise<boolean> => {
    if (!window.electronAPI) {
      return false;
    }
    ...
    try {
      ...
      const response = (await window.electronAPI.addMarketplace(request)) as AddMarketplaceResponse;
   ...
   */
  const removeMarketplace = useCallback(async (name: string): Promise<boolean> => {
    if (!window.electronAPI) {
      return false;
    }
    ...
    try {
      ...
      const response = (await window.electronAPI.removeMarketplace(request)) as RemoveMarketplaceResponse;
   ...
   */
  const installPlugin = useCallback(async (pluginName: string, marketplaceName: string): Promise<boolean> => {
    if (!window.electronAPI) {
      return false;
    }
  ...
  const uninstallPlugin = useCallback(async (pluginId: string): Promise<boolean> => {
    ...
    try {
      ...
      const response = (await window.electronAPI.uninstallPlugin(request)) as UninstallPluginResponse;
  ...
  const getGitHubRepoInfo = useCallback(async (repoUrl: string): Promise<GitHubRepoInfo | null> => {
    ...
    try {
      ...
      const response = (await window.electronAPI.getGitHubRepoInfo(request)) as GetGitHubRepoInfoResponse;
   ...
   */
  const getPluginHealth = useCallback(async (plugin: MarketplacePlugin | InstalledPlugin): Promise<PluginHealthScore | null> => {
    if (!window.electronAPI) {
      return null;
    }
    ...
    try {
      ...
      const response = (await window.electronAPI.getPluginHealth(request)) as GetPluginHealthResponse;
